...
pub fn distribution_with_literal() {
    let mock = mocks::distribution_mock("test", "test_value", EMPTY_TAGS);
    DatadogWrapper::new(mock, true).do_distribution("test", "test_value", EMPTY_TAGS);
}

#[test]
pub fn distribution_with_type() {
    let mock = mocks::distribution_mock("test1_event", "test_value", EMPTY_TAGS);
    DatadogWrapper::new(mock, true).do_distribution(TestEvent::Test1, "test_value", EMPTY_TAGS);
}
