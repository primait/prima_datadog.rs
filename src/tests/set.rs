...
pub fn set_with_literal() {
    ...
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_set("test", "test_value", EMPTY_TAGS);
}

#[test]
pub fn set_with_type() {
    let mock = mocks::set_mock("test1_event", "test_value", &[]);
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_set(TestEvent::Test1, "test_value", EMPTY_TAGS);
}

#[test]
pub fn set_with_literal_and_tags() {
    let mock = mocks::set_mock("test", "test_value", &["added:tag", "env:test"]);
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_set(
        "test",
        "test_value",
