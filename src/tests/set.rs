...
pub fn set_with_literal() {
    ...
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_set("test", "test_value", vec![]);
}

#[test]
pub fn set_with_type() {
    let mock = mocks::set_mock("test1_event", "test_value", &[]);
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_set(TestEvent::Test1, "test_value", vec![]);
}
