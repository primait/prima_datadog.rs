...
pub fn timing_with_literal() {
    ...
    Datadog::new(mock, true, TrackerConfiguration::new()).do_timing("test", 10, vec![]);
}

#[test]
pub fn timing_with_type() {
    let mock = mocks::timing_mock("test1_event", 10, &[]);
    Datadog::new(mock, true, TrackerConfiguration::new()).do_timing(TestEvent::Test1, 10, vec![]);
}

#[test]
pub fn timing_with_literal_and_tags() {
    let mock = mocks::timing_mock("test", 10, &["added:tag", "env:test"]);
    Datadog::new(mock, true, TrackerConfiguration::new()).do_timing("test", 10, vec!["added:tag".to_string()]);
}

#[test]
pub fn timing_with_type_and_tags() {
    let mock = mocks::timing_mock("test1_event", 10, &["added:tag", "env:test"]);
    Datadog::new(mock, true, TrackerConfiguration::new()).do_timing(
        TestEvent::Test1,
        10,
