...
pub fn event_with_literal_and_tags() {
    ...
    Datadog::new(mock, true, ["env:test"]).event("test", "test_value", vec!["added:tag".to_string()]);
}

#[test]
pub fn event_with_type_and_tags() {
    let mock = mocks::event_mock("test1_event", "test_value", &["added:tag", "env:test"]);
    Datadog::new(mock, true, ["env:test"]).event(common::TestEvent::Test1, "test_value", vec!["added:tag".to_string()]);
}
