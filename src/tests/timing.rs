...
pub fn timing_with_literal() {
    ...
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_timing("test", 10, vec![]);
}

#[test]
pub fn timing_with_type() {
    let mock = mocks::timing_mock("test1_event", 10, &[]);
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_timing(TestEvent::Test1, 10, vec![]);
}
