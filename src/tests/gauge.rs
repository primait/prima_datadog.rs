...
pub fn gauge_with_literal() {
    ...
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_gauge("test", "test_value", vec![]);
}

#[test]
pub fn gauge_with_type() {
    let mock = mocks::gauge_mock("test1_event", "test_value", &[]);
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_gauge(TestEvent::Test1, "test_value", vec![]);
}
