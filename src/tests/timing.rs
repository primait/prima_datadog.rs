...
pub fn timing_with_literal() {
    ...
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_timing("test", 10, EMPTY_TAGS);
}

#[test]
pub fn timing_with_type() {
    let mock = mocks::timing_mock("test1_event", 10, &[]);
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_timing(TestEvent::Test1, 10, EMPTY_TAGS);
}

#[test]
pub fn timing_with_literal_and_tags() {
    let mock = mocks::timing_mock("test", 10, &["added:tag", "env:test"]);
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_timing(
        "test",
        10,
