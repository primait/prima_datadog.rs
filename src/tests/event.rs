...
pub fn event_with_literal() {
    ...
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_event("test", "test_value", EMPTY_TAGS);
}

#[test]
pub fn event_with_type() {
    let mock = mocks::event_mock("test1_event", "test_value", &[]);
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_event(
        TestEvent::Test1,
        "test_value",
        EMPTY_TAGS,
    );
