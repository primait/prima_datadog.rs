...
pub fn event_with_literal() {
    ...
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_event("test", "test_value", vec![]);
}

#[test]
pub fn event_with_type() {
    let mock = mocks::event_mock("test1_event", "test_value", &[]);
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_event(TestEvent::Test1, "test_value", vec![]);
}
