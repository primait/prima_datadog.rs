...
pub fn count_with_literal() {
    ...
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_count("test", 10, vec![]);
}

#[test]
pub fn count_with_type() {
    let mock = mocks::count_mock("test1_event", 10, &[]);
    Datadog::new(mock, true, TagTrackerConfiguration::new()).do_count(TestEvent::Test1, 10, vec![]);
}
