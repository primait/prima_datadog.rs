...
pub fn gauge_with_literal() {
    ...
    DatadogWrapper::new(mock, true, TagTrackerConfigurationWrapper::new()).do_gauge("test", "test_value", EMPTY_TAGS);
}

#[test]
pub fn gauge_with_type() {
    let mock = mocks::gauge_mock("test1_event", "test_value", &[]);
    DatadogWrapper::new(mock, true, TagTrackerConfigurationWrapper::new()).do_gauge(
        TestEvent::Test1,
        "test_value",
        EMPTY_TAGS,
    );
