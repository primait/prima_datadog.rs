...
pub fn histogram_with_literal() {
    ...
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_histogram("test", "test_value", EMPTY_TAGS);
}

#[test]
pub fn histogram_with_type() {
    let mock = mocks::histogram_mock("test1_event", "test_value", &[]);
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_histogram(
        TestEvent::Test1,
        "test_value",
        EMPTY_TAGS,
    );
