...
pub fn incr_with_literal() {
    ...
    DatadogWrapper::new(mock, true, TagTrackerConfigurationWrapper::new()).do_incr("test", EMPTY_TAGS);
}

#[test]
pub fn incr_with_type() {
    let mock = mocks::incr_mock("test1_event", &[]);
    DatadogWrapper::new(mock, true, TagTrackerConfigurationWrapper::new()).do_incr(TestEvent::Test1, EMPTY_TAGS);
}
...
pub fn incr_with_literal_and_tags() {
    ...
    DatadogWrapper::new(mock, true, TagTrackerConfigurationWrapper::new())
        .do_incr("test", vec!["added:tag".to_string()]);
}
