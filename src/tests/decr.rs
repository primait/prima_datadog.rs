...
pub fn decr_with_literal() {
    ...
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_decr("test", EMPTY_TAGS);
}

#[test]
pub fn decr_with_type() {
    let mock = mocks::decr_mock("test1_event", &[]);
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_decr(TestEvent::Test1, EMPTY_TAGS);
}

#[test]
pub fn decr_with_literal_and_tags() {
    let mock = mocks::decr_mock("test", &["added:tag", "env:test"]);
    DatadogWrapper::new(mock, true, TagTrackerConfiguration::new()).do_decr("test", vec!["added:tag".to_string()]);
}
